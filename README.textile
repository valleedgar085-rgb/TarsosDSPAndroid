h1. Note: you don't need to use this fork to run TarsosDSP on Android. As of version 2.0, TarsosDSP runs on Android natively. See http://0110.be/posts/TarsosDSP_on_Android_-_Audio_Processing_in_Java_on_Android
...
h3. TarsosDSP with Gradle (modern build)

The repository now ships with the Gradle wrapper, so no additional tooling is required. From the project root run:

<pre><code>./gradlew build
</code></pre>

This produces the library, sources, and Javadoc jars in @build/libs@. The legacy JUnit test suite is skipped by default because this trimmed code drop no longer contains some of the classes it targets. To attempt compiling and running those tests anyway, add the @includeLegacyTests@ project property:

<pre><code>./gradlew build -PincludeLegacyTests
</code></pre>

Additional useful tasks: publish to your local Maven cache via @./gradlew publishToMavenLocal@ or clean outputs with @./gradlew clean@.
